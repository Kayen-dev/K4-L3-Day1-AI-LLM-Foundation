...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        r4o = (r.get("gpt4o_response", "")[:37] + "...") if len(r.get("gpt4o_response", "")) > 40 else r.get("gpt4o_response", "")
        rmini = (r.get("mini_response", "")[:37] + "...") if len(r.get("mini_response", "")) > 40 else r.get("mini_response", "")
        lat4o = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        latmini = f"{r.get('mini_latency', 0.0):.2f}s"
        rows.append(f"{p:<40} | {r4o:<40} | {rmini:<40} | {lat4o:<15} | {latmini:<15}")
    return "\n".join(rows)
